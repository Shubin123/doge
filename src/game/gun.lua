...
function Gun:drawBarrel()
    ...
    if self.blockInstance then
        ...
        local vx = GunTool.lastAimDirection.x
        local vy = GunTool.lastAimDirection.y
        
        self.lastAimDirection.x = GunTool.lastAimDirection.x
        self.lastAimDirection.y = GunTool.lastAimDirection.y
...
function GunTool.load(world)
    ...
    GunTool.totalGuns = #GunTool.guns
...
function GunTool.update(dt)
    ...
    gun.currentVel.x, gun.currentVel.y = mymath.lerpVec2({ gun.currentVel.x, gun.currentVel.y },
        { gun.lastAimDirection.x, gun.lastAimDirection.y }, 0.1)
...
function GunTool.keypressed(key)
    ...
    if weaponNum and weaponNum >= 1 and weaponNum <=  GunTool.totalGuns then
