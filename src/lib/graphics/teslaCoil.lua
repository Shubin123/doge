    ...
    -- if var.graphics_high then
        ...
        whiteNeon(function()
            for _, bolt in ipairs(state.bolts) do
                for i = 1, #bolt - 1 do
                    drawPixelLine(camera.pos.x + bolt[i].x * camera.zoom, camera.pos.y + bolt[i].y * camera.zoom,
                        camera.pos.x + bolt[i + 1].x * camera.zoom, camera.pos.y + bolt[i + 1].y * camera.zoom)
                end
            end
        end)
        love.graphics.pop()
...
function teslaCoil.fireAt(x, y, world)
    ...
    for i = 1, config.spark_count do
        ...
        local random_spread = (math.random() - 0.5) *
        (config.endpoint_spread * math.pi / 180)                                               -- Random spread in radians
